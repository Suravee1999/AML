*** Settings ***
Library     SeleniumLibrary
Resource     ปปง1-03_va.robot
Resource    ../AMLO Variables.robot

*** Variables ***

${UPLOAD_FILE_PATH}         D:\\AML_Project\\File\\รูปบัตรประชาชน.png

*** Keywords ***
อัพโหลดไฟล์
        Sleep    1s
        Choose File    ${อัพโหลด_xpath}     ${UPLOAD_FILE_PATH}


เลือกรายการ_ปปง1-03
        Wait Until Element Is Visible    ${ประเภทรายงาน}
        Click Element    ${ประเภทรายงาน}
        Click Element    ${ปปง.1-03}
#*******************************************************************

ยื่นคำร้องอื่นๆ & ไม่ใช่ลูกค้าธนาคาร
        Click Element    ${รายงานเนื่องจาก_รายงานตัวช้า}
        Click Element    ${รายงานเนื่องจาก_อื่นๆ}
        Input Text    ${รายงานเนื่องจาก_อื่นๆ_ระบุ}    ${รายงานเนื่องจาก_อื่นๆ}
        Click Element    ${ค้าหาข้อมูลจากลูกค้าด้วย_บัตรประจำตัว_ไม่ใช้ลูกค้าธนาคาร}
        Click Element    ${ยื่นคำร้องManual_ถัดไป}
        
ยื่นคำร้องรายงานล่าช้า & พบพฤติกรรม &ไม่ใช่ลูกค้าธนาคาร

        Click Element    ${รายงานเนื่องจาก_รายงานตัวช้า}
        Click Element    ${แหล่งที่มาของเหตุอันควรสงสัย_พบพฤติกรรม/รายการทำธุรกรรมของลูกค้าที่มีเหตุอันควรสงสัย}
        Click Element    ${ค้าหาข้อมูลจากลูกค้าด้วย_บัตรประจำตัว_ไม่ใช้ลูกค้าธนาคาร}
        Click Element    ${ยื่นคำร้องManual_ถัดไป}
ยื่นคำร้องรายงานล่าช้า & แหล่งที่มาของเหตุอันควรสงสัย &ไม่ใช่ลูกค้าธนาคาร
        Click Element    ${รายงานเนื่องจาก_รายงานตัวช้า}
        Click Element    ${แหล่งที่มาของเหตุอันควรสงสัย}
        Click Element    ${ค้าหาข้อมูลจากลูกค้าด้วย_บัตรประจำตัว_ไม่ใช้ลูกค้าธนาคาร}
        Click Element    ${ยื่นคำร้องManual_ถัดไป}
#*******************************************************************
ส่วนที่1 ระบุข้อมูล(ทำธุรกรรมด้วยตนเอง)
        เรียกเลขประจำตัวประชาชน
        Wait Until Element Is Visible    ${เหตุผลการรายงาน Mannual_เหตุผลในการรายงาน}
        Input Text    ${เหตุผลการรายงาน Mannual_เหตุผลในการรายงาน}    ${เหตุผลในการรายงาน}
        Click Element    ${1.1_คำนำหน้าชื่อ}
        Click Element    ${1.1_คำนำหน้าชื่อ_เลือก}
        Input Text    ${1.1_ชื่อ}    ${ชื่อ}
        Input Text    ${1.1_นามสกุล}    ${นามสกุล}
        Input Text    ${1.1_เอกสารประจำตัว_เลขที่}    ${random_number}
        Click Element    ${1.1_เอกสารประจำตัว_ทำธุรกรรมด้วยตนเอง}
        Input Text    ${1.2_ที่อยู่}    ${ที่อยู่}
        Click Element    ${1.2_ประเทศ}
        Click Element    ${1.2_ที่อยู่_ประเทศ_ไทย}
        Click Element    ${1.2_ที่อยู่_ประเทศ_จังหวัด}
        Click Element    ${1.2_ที่อยู่_ประเทศ_จังหวัด_เลือก}
        Click Element    ${1.2_ที่อยู่_ประเทศ_อำเภอ}
        Click Element    ${1.2_ที่อยู่_ประเทศ_อำเภอ_เลือก}
        Click Element    ${1.2_ที่อยู่_ประเทศ_ตำบล}
        Click Element    ${1.2_ที่อยู่_ประเทศ_ตำบล_เลือก}
        Input Text    ${1.2_ที่อยู่_ประเทศ_รหัสไปรษณีย์_เลข}    ${รหัสไปรษณีย์}
        Click Element    ${1.3อาชีพ}
        Click Element    ${1.3อาชีพ_เลือก}
        Input Text    ${1.5_เลขที่}    ${random_number}
ส่วนที่1 ระบุข้อมูล(ทำธุรกรรมแทนผู้อื่น)
        เรียกเลขประจำตัวประชาชน
        Wait Until Element Is Visible    ${เหตุผลการรายงาน Mannual_เหตุผลในการรายงาน}
        Input Text    ${เหตุผลการรายงาน Mannual_เหตุผลในการรายงาน}    ${เหตุผลในการรายงาน}
        Click Element    ${1.1_คำนำหน้าชื่อ}
        Click Element    ${1.1_คำนำหน้าชื่อ_เลือก}
        Input Text    ${1.1_ชื่อ}    ${ชื่อ}
        Input Text    ${1.1_นามสกุล}    ${นามสกุล}
        Input Text    ${1.1_เอกสารประจำตัว_เลขที่}    ${random_number}
        Click Element    ${1.1_ผู้ทำธุรกรรม_ทำธุรกรรมแทนผู้อื่น}
        Input Text    ${1.2_ที่อยู่}    ${ที่อยู่}
        Click Element    ${1.2_ประเทศ}
        Click Element    ${1.2_ที่อยู่_ประเทศ_ไทย}
        Click Element    ${1.2_ที่อยู่_ประเทศ_จังหวัด}
        Click Element    ${1.2_ที่อยู่_ประเทศ_จังหวัด_เลือก}
        Click Element    ${1.2_ที่อยู่_ประเทศ_อำเภอ}
        Click Element    ${1.2_ที่อยู่_ประเทศ_อำเภอ_เลือก}
        Click Element    ${1.2_ที่อยู่_ประเทศ_ตำบล}
        Click Element    ${1.2_ที่อยู่_ประเทศ_ตำบล_เลือก}
        Input Text    ${1.2_ที่อยู่_ประเทศ_รหัสไปรษณีย์_เลข}    ${รหัสไปรษณีย์}
        Click Element    ${1.3อาชีพ}
        Click Element    ${1.3อาชีพ_เลือก}
        Input Text    ${1.5_เลขที่}    ${random_number}

ส่วนที่1 บันทึกและไปหน้าถัดไป
        Click Element    ${ส่วนที่1_บีนทึก}
        Run Keyword And Ignore Error    (ปุ่มตกลง)
        Wait Until Element Is Visible    ${ส่วนที่1_ถัดไป}
        Click Element    ${ส่วนที่1_ถัดไป}
(ปุ่มตกลง)
        Wait Until Element Is Enabled    ${ส่วนที่1_ตกลง}    timeout=10s
        Wait Until Element Is Visible    ${ส่วนที่1_ตกลง}    timeout=10s
        Click Element    ${ส่วนที่1_ตกลง}

ส่วนที่2 ปุ่มเพื่ม
        Wait Until Element Is Visible    ${ส่วนที่2_ปุ่มเพื่ม}
       Click Element    ${ส่วนที่2_ปุ่มเพื่ม}
ส่วนที่2 ไม่ใช่ลูกค้าธนาคาร
        Wait Until Element Is Visible    ${ส่วนที่2_ไม่ใช่ลูกค้าธนาคาร}
        Click Element    ${ส่วนที่2_ไม่ใช่ลูกค้าธนาคาร}
ส่วนที่ 2 ระบุข้อมูล(บุคคลธรรมดา)
        เรียกเลขประจำตัวประชาชน
        Wait Until Element Is Visible    ${ส่วนที่2_ผู้ทำธุรกรรม_ผู้มอบอำนาจ}
        Click Element    ${ส่วนที่2_ผู้ทำธุรกรรม_ผู้มอบอำนาจ}
        Click Element    ${ส่วนที่2_ประเภทผู้ทำธุรกรรม_บุคคลธรรมดา}
        Click Element    ${2.1_คำนำหน้าชื่อ}
        Click Element    ${2.1_คำนำหน้าชื่อ_เลือก}
        Input Text    ${2.1_ชื่อ}    ${ชื่อ_ส่วน2}
        Input Text    ${2.1_นามสกุล}    ${นามสกุล_ส่วน2}
        Input Text    ${2.1_เอกสารประจำตัว_เลขที่}    ${random_number}
        Input Text    ${2.2_ที่อยู่}    ${ที่อยู่_ส่วน2}
        Click Element    ${2.2_ประเทศ}
        Click Element    ${2.2_ที่อยู่_ประเทศ_ไทย}
        Click Element    ${2.2_ที่อยู่_ประเทศ_จังหวัด}
        Click Element    ${2.2_ที่อยู่_ประเทศ_จังหวัด_เลือก}
        Click Element    ${2.2_ที่อยู่_ประเทศ_อำเภอ}
        Click Element    ${2.2_ที่อยู่_ประเทศ_อำเภอ_เลือก}
        Click Element    ${2.2_ที่อยู่_ประเทศ_ตำบล}
        Click Element    ${2.2_ที่อยู่_ประเทศ_ตำบล_เลือก}
        Input Text    ${2.2_ที่อยู่_ประเทศ_รหัสไปรษณีย์_เลข}    ${รหัสไปรษณีย์_ส่วน2}
        Click Element    ${2.3อาชีพ}
        Click Element    ${2.3อาชีพ_เลือก}
        Input Text    ${2.5_เลขที่}    ${random_number}

ส่วนที่ 2 ระบุข้อมูล(นิติบุคคล)
        เรียกเลขประจำตัวประชาชน
        Wait Until Element Is Visible    ${ส่วนที่2_ผู้ทำธุรกรรม_ผู้มอบหมาย}
        Click Element    ${ส่วนที่2_ผู้ทำธุรกรรม_ผู้มอบหมาย}
        Click Element    ${ส่วนที่2_ประเภทผู้ทำธุรกรรม_นิติบุคคล}
        Input Text    ${2.1_ชื่อ}    ${ชื่อ_ส่วน2}
        Input Text    ${2.1_เอกสารประจำตัว_เลขที่}    ${random_number}
        Input Text    ${2.2_ที่อยู่}    ${ที่อยู่_ส่วน2}
        Click Element    ${2.2_ประเทศ}
        Click Element    ${2.2_ที่อยู่_ประเทศ_ไทย}
        Click Element    ${2.2_ที่อยู่_ประเทศ_จังหวัด}
        Click Element    ${2.2_ที่อยู่_ประเทศ_จังหวัด_เลือก}
        Click Element    ${2.2_ที่อยู่_ประเทศ_อำเภอ}
        Click Element    ${2.2_ที่อยู่_ประเทศ_อำเภอ_เลือก}
        Click Element    ${2.2_ที่อยู่_ประเทศ_ตำบล}
        Click Element    ${2.2_ที่อยู่_ประเทศ_ตำบล_เลือก}
        Input Text    ${2.2_ที่อยู่_ประเทศ_รหัสไปรษณีย์_เลข}    ${รหัสไปรษณีย์_ส่วน2}
        Click Element    ${2.3อาชีพ}
        Click Element    ${2.3อาชีพ_เลือก}
        Input Text    ${2.5_เลขที่}    ${random_number}
ส่วนที่2 บันทึกและไปหน้าถัดไป
        Click Element    ${ส่วนที่1_บีนทึก}
        Sleep    1s

ส่วนที่2 ถัดไป
        Wait Until Element Is Visible    ${ส่วนที่2_ถัดไป}
        Click Element    ${ส่วนที่2_ถัดไป}
##################################
ข้อเท็จจริง(บุคคลธรรมดา)
        Click Element        ${ชื่อเจ้าของบัญชี_xpath}
        Click Element    ${คำนำหน้า_xpath}
        Wait Until Element Is Visible    ${3.คำนำหน้าชื่อ_เลือก}
        Click Element    ${3.คำนำหน้าชื่อ_เลือก}
        Input Text    ${3.ชื่อ_xpath}    ${ส่วนที่3_ชือ}
        Input Text    ${3.นามสกุล_xpath}    ${ส่วนที่3_นามสกุล}
ข้อเท็จจริง(นิติบุคคล)
        Input Text    ${3.ชื่อ_xpath}    ${ส่วนที่3_ชือ}
#########################################



ส่วนที่ 3 ข้อเท็จจริงเกี่ยวกับการทำธุรกรรม
        Wait Until Element Is Visible    ${วันที่ทำธุรกรรม_xpath}
        Input Text    ${วันที่ทำธุรกรรม_xpath}     ${วันที่ทำธุรกรรม}
        Input Text    ${มูลค่าของธุรกรรม_xpath}    ${มูลค่าของธุรกรรม}
        Click Element    ${ประเภทธุรกรรม_xpath}
        Input Text    ${รายละเอียดของธุรกรรม_xpath}    ${รายละเอียดของธุรกรรม}
        Input Text    ${เลขที่บัญชีที่ทำธุรกรรม_xpath}    ${เลขที่บัญชีที่ทำธุรกรรม}
        ข้อเท็จจริง(บุคคลธรรมดา)
        Click Element    ${วัตถุประสงค์ของการทำธุรกรรม_xpath}
        Click Element    ${วัตถุประสงค์ของการทำธุรกรรม_เลือก}



ส่วนที่ 3 ไม่พบความผิดปกติ
            Wait Until Element Is Visible    ${ไม่พบความผิดปกติ}
            Click Element    ${ไม่พบความผิดปกติ}
            Click Element    ${ขออนุมัติรายงานธุรกรรมที่มีเหตุอันควรสงสัย}
             Input Text    ${3.เหตุผลประกอบการพิจารณา}    ${เหตุผลประกอบการพิจารณา}

ส่วนที่ 3 พบความผิดปกติ
            Wait Until Element Is Visible    ${พบความผิดปกติ}
            Click Element    ${พบความผิดปกติ}
#            Click Element    ${ขออนุมัติรายงานธุรกรรมที่มีเหตุอันควรสงสัย}
            Input Text    ${3.เหตุผลประกอบการพิจารณา}    ${เหตุผลประกอบการพิจารณา}



ส่วนที่ 3 บันทึกข้อมูล

           เก็บรหัสCIF
           Click Element    ${ส่วนที่1_บีนทึก}
           Run Keyword And Ignore Error    (ปุ่มตกลง)

ส่วนที่ 3 ขออนุมัติ
           Click Element    ${ขออนุมัติ}
           Wait Until Element Is Visible    ${ส่วนที่3_ยืนยัน}
           Click Element    ${ส่วนที่3_ยืนยัน}
           Run Keyword And Ignore Error    (ปุ่มตกลง)

